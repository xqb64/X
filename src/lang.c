...
struct ParseFnResult unary(struct Parser *parser)
{
  if (match(parser, 1, TOKEN_MINUS)) {
    ...
    /* Constant fold the negative literal */
    if (right_result.as.expr.kind == EXPR_LITERAL &&
      ...
      if (is_unsigned) {
        ...
        if (lit.type.kind == U8_T) {
          uval = lit.as.u8;
        } else if (lit.type.kind == U16_T) {
          uval = lit.as.u16;
        } else if (lit.type.kind == U32_T) {
          uval = lit.as.u32;
        } else if (lit.type.kind == U64_T) {
          val = lit.as.u64;
        }
        val = -(long long) uval;
      } else {
        long long sval = 0;
        if (lit.type.kind == I8_T) {
          sval = lit.as.i8;
        } else if (lit.type.kind == I16_T) {
          sval = lit.as.i16;
        } else if (lit.type.kind == I32_T) {
          sval = lit.as.i32;
        } else if (lit.type.kind == I64_T) {
          sval = lit.as.i64;
        }
        val = -sval;
      }
      ...
      right_result.as.expr.type = new_type;
      right_result.as.expr.as.literal.type = new_type;

      if (new_type.kind == I8_T) {
        right_result.as.expr.as.literal.as.i8 = (char) val;
      } else if (new_type.kind == I16_T) {
        right_result.as.expr.as.literal.as.i16 = (short) val;
      } else if (new_type.kind == I32_T) {
        right_result.as.expr.as.literal.as.i32 = (int) val;
      } else {
